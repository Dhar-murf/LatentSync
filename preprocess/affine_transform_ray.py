...
def affine_transform_multi_gpus(input_dir, output_dir, temp_dir, resolution, num_workers):
    os.system("rm -r /home/ubuntu/preprocess/affine_transformed/")
    ...
    paths_to_process = sorted(paths)[:1000]

    processes = []
    i=0
    futures = [
    func.remote(path, os.path.join(temp_dir, f"process_{i}"), i, resolution) for path in paths_to_process
    ]
    print("starting")
...
if __name__ == "__main__":
    input_dir = "/home/ubuntu/preprocess/segmented"
    output_dir = "/home/ubuntu/preprocess/affine_transformed"
    temp_dir = "temp"
    resolution = 256
